...
class MyPDO extends PDO {    
    ...
    public static function createQMarks ($array) {
        return '(' . implode(', ', array_fill(0, count($array), '?')) . ')';
    }
    
    
    public static function makeQueryTagsCorr ($colSel,$sources) {
        $selSel = '';
        for ($i=1;$i<=2;$i++) {
          $selSel .= "CONCAT(t0.sc{$i},";
            foreach ($sources as $source2) {
              $selSel .= "IFNULL(t0.fk_{$source2}_id{$i},''),";
            }
            $selSel = substr($selSel,0,-1) . ") AS concatid{$i},";
        }
        ...
        foreach ($colSel as $col) {
          for ($i=1;$i<=2;$i++) {
            ...
            foreach ($sources as $source) {
              $querySel .= "IFNULL({$source}{$i}.{$col},''),";
            }
            $querySel = substr($querySel,0,-1).") AS {$i}_{$col},";
          }
        }
        $querySel = substr($querySel,0,-1).' ';
        
        $joinSel = '';
        foreach ($sources as $source) {
          for ($i=1;$i<=2;$i++) {
            $joinSel .= "LEFT JOIN tags_{$source} {$source}{$i}
                              ON CONCAT(t0.sc{$i},";
                foreach ($sources as $source2) {
                  $joinSel .= "IFNULL(t0.fk_{$source2}_id{$i},''),";
                }
                $joinSel = substr($joinSel,0,-1) . ") = CONCAT('{$source}',{$source}{$i}.id)";
          }
        }
        
        $q = array('selSel' => $selSel,'querySel' => $querySel,'joinSel' => $joinSel);
        return $q;
